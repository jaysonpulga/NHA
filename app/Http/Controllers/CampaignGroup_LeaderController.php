...
class CampaignGroup_LeaderController extends Controller
{
	...
	public function getCoordinatorbelongtocityandbarangay(Request $request)
	{
		...
		$select ="t1.coordinator_id,t2.name,t2.vin_number";
	...
	public function LoadLeader(Request $request)
	{
		...
		$select ="t1.leader_id,t1.vin_number,t2.name as leader,t7.name as coordinator,t3.name as city, t4.name as barangay,t5.name as province";
		...
		->leftJoin('votersinfomations AS t7','t7.vin_number','=','t6.vin_number')
	...
	public function AssignLeader(Request $request)
	{
		...
		$Leaders->vin_number     =  $request->leader;
		$Leaders->save();
